...
- **Thumbnails** — fast generated previews for images, with graceful fallback icons for video, audio, PDF, fonts, design files, and archives.
...
There are no published releases yet. Every merge to `main` builds a Windows package via GitHub Actions (`.github/workflows/build.yml`); download it from that run's **Artifacts** on the [Actions page](https://github.com/lzitser23/maat/actions):
...
Maat writes to two separate directories on Windows — back up, migrate, or troubleshoot using the first one; the second is disposable.

| Path | What's there | Owned by |
| --- | --- | --- |
| `%APPDATA%\MaatNative` (`C:\Users\<you>\AppData\Roaming\MaatNative`) | Your data: the SQLite catalog (boards, tags, notes) and the content-addressed managed asset store. This is what you back up or migrate. | Maat (`src-zig/main.zig`) |
| `%LOCALAPPDATA%\com.lzitser.maat-native` (`C:\Users\<you>\AppData\Local\com.lzitser.maat-native`) | Window position/size restore state and diagnostic logs, keyed by the app's bundle id. Nothing here is user data — safe to delete any time; Maat just re-centers the window and recreates it. | Native SDK shell |
...
`-- .github/      # CI workflow (web checks + Windows package on merge to main)
```
